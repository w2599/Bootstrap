...
void initFromSwiftUI()
{
    ...
    [AppDelegate addLogText:[NSString stringWithFormat:Localized(@"build-time: %@\nPlease retest any issues using the official version before reporting them to the developers."),@"26/03/30 21:40:59"]];
        ...
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            ...
            if(status == 0 || status == EINVAL) {
                ...
                if(status == 0) {
                    ...
                    // 如果是包含Library/Fonts, 就注销一次
                    if([mountPath containsString:@"Library/Fonts"]) {
                        respringAction();
                    }
                    [AppDelegate showMesage:Localized(@"Done") title:@""];
                } else {
